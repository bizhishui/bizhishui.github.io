...
{:.no_toc}
The first type of sed script demonstrates making a series of edits in a file. Here is an example:

```
    #a sed script
    s/^$/.LP/           #subsitute blank line with .LP
    /^+  */d            #delete line start with + and at least one space
    s/^  *//            #remove any sequence of spaces found at the beginning of a line (>=1 space)
    s/  */ /g           #replace any string of consecutive spaces with a single space
    s/\.  */.  /g       #replace a period followed by any number of spaces with a period followed by two spaces
```
